...
- **콘텐츠 경로:** 성과 리뷰 보고서는 반드시 `content/picks-feedback/` 폴더 내에 저장되어야 합니다.
- **파일 확장자:** 반드시 `.mdx` 확장자를 사용해야 웹사이트에 정상 노출됩니다.
- **필수 데이터(Frontmatter):** 모든 파일 상단에는 `title`, `date`, `summary`, `tags` 정보가 반드시 포함되어야 합니다.

## 2. 실행 시점 (Trigger)
- 성과 리뷰 보고서 생성이 완료된 후 사용자가 **"Push 하자"**, **"배포해줘"**, **"업로드 진행"**과 같은 명령을 내릴 때 실행합니다.

## 3. 배포 표준 절차 (Step-by-Step)
1. **파일 규격 확인:** 생성된 보고서가 `.mdx` 확장자이며 `content/picks-feedback/` 폴더에 위치하는지 확인합니다.
2. **로컬 Git 상태 감지:** 현재 폴더가 Git 저장소인지 확인하고, 변경된 리포트 파일 리스트를 추출합니다.
3. **변경 사항 스테이징:** `git add` 명령을 사용하여 배포할 파일을 준비합니다.
4. **커밋 메시지 작성:** `[Review] YYYY-MM-DD 성과 리뷰 보고서 신규 배포` 형식을 따릅니다.
5. **원격 저장소 전송:** `origin main` 브랜치로 푸시를 수행하여 배포를 완료합니다.
