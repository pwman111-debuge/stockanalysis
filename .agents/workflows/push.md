...
### 1단계: 대상 파일 식별 및 프론트매터 검증
1. 현재 작업 디렉토리에서 최근에 생성된 보고서 파일(보통 `genesis_report_*.mdx` 또는 `.md`)을 찾습니다.
2. **[🚨 중요] 프론트매터(Frontmatter) 형식 검증:**
   - `date`, `category`, `title`, `summary` 등 모든 값에 **따옴표(`" "`)**가 포함되어 있는지 확인합니다.
   - 따옴표가 누락된 경우, 배포 시 웹사이트 노출이 되지 않으므로 반드시 수정 후 진행해야 합니다.
   - 예시: `date: "2026-03-27"`, `category: "Genesis"` (O) / `date: 2026-03-27` (X)
